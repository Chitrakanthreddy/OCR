...
from processing_tools.east.east import EAST_OUTPUT_LAYERS
from processing_tools.east.east import decode_predictions
from processing_tools.helpers import cleanup_text
import pytesseract
import numpy as np
import argparse
import time
...
ap.add_argument("-e", "--east", required=True, help="path to input EAST text detector")
ap.add_argument("-w", "--width", type=int, default=320, help="resized image width (should be multiple of 32)")
ap.add_argument("-t", "--height", type=int, default=320, help="resized image height (should be multiple of 32)")
ap.add_argument("-c", "--min-conf", type=float, default=0.5, help="minimum probability required to inspect a text region")
ap.add_argument("-n", "--nms-thresh", type=float, default=0.4, help="non-maximum suppression threshold")
ap.add_argument("-p", "--padding", type=float, default=0.0, help="amount of padding to add to each border of ROI")
ap.add_argument("-s", "--sort", type=str, default="top-to-bottom", help="whether we sort bounding boxes left-to-right or top-to-bottom")
